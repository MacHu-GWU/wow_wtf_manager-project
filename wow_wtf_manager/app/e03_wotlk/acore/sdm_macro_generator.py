...
sdm_macro.py 代码的逻辑, 以便于人类编辑了 Yaml 目录后自动更新这个模块.
"""
...
for dir in dir_sdm.select_dir(recursive=False):
    for p in Path.sort_by_fname(dir.select_by_ext(ext=".yml")):
        # example: 02-paladin/0-common/11101-consumable.yml
        relpath = p.relative_to(dir_sdm)
        # example: sdm_02_paladin____0_common____11101_consumable
        key = "sdm_" + (
            str(relpath)[:-4]
            .replace("-", "_")
            .replace("/", "____")
        ...
        value = "dir_wotlk_acore_project.joinpath({}, {})".format(
            '"SuperDuperMacro"', ", ".join([f'"{part}"' for part in relpath.parts])
        )
        line = f"    {key} = SDMMacroFile(path={value})"
        lines.append(line)
